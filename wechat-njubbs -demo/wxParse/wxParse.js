...
function wxParse(bindName = 'wxParseData', type = 'html', data = '<div class="color:red;">数据不能为空</div>', target, imagePadding) {
  ...
  var bindData = {};
  var allReply = that.data.allReply;
  if (bindName == 'article') {
    bindData[bindName] = transData;
    that.setData(bindData)
  } else {
    allReply.push(transData);
    that.setData({
      allReply: allReply
    })
  }
...
function calMoreImageInfo(e, idx, that, bindName) {
  ...
  if (bindName != 'article'){
    var num = bindName.substr (5);
    temData = that.data.allReply[num];
  }
...
function wxAutoImageCal(originalWidth, originalHeight, that, bindName) {
  ...
  wx.getSystemInfo({
    success: function (res) {
      var padding;
      if (bindName == 'article'){
        padding = that.data[bindName].view.imagePadding;
      }else{
        var num = bindName.substr(5);
        padding = that.data.allReply[num].view.imagePadding;
      }
